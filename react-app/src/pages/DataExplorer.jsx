...
const DataExplorer = () => {
  ...
  const handleExecuteQuery = async (queryText, targetPage = 1) => {
    setIsLoading(true);
    setError(null);
  ...
  return (
    <div className="dark-theme" style={{ 
      ...
      <header style={{ marginBottom: '32px', position: 'relative', zIndex: 1, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          ...
          <p style={{ color: '#94a3b8', margin: 0, fontSize: '15px', maxWidth: '600px', lineHeight: '1.6' }}>
            Enter a natural language query. The intelligence engine will autonomously search the datastore to retrieve live case records, entities, and evidence.
          </p>
        </div>
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        <QueryInput 
          onExecute={(q) => handleExecuteQuery(q, 1)} 
          isLoading={isLoading} 
        />
        ...
        {queryState.results && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', animation: 'fadeIn 0.5s ease-out' }}>
            ...
            <ResultRenderer 
              ...
              page={currentPage}
              pageSize={pageSize}
              onPageChange={(newPage) => handleExecuteQuery(queryState.query, newPage)}
            />
          </div>
        )}
